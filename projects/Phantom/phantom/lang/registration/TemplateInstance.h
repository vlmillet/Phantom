...
#define PHANTOM_TEMPLATE_INSTANCE(...)                                                                                 \
    ...
    {                                                                                                                  \
    static_assert(                                                                                                     \
    !std::is_same<phantom::lang::Type*, decltype(phantom::TypeOf(_PHNTM_TypeOfVal(__VA_ARGS__)))>::value,              \
    "no template lang found for '" PHANTOM_PP_QUOTE(                                                                   \
    __VA_ARGS__) "' ; if using haunt, the associated '.hxx' #include is probably missing");                            \
    PHANTOM_REGISTER(End) { phantom::TypeOf(_PHNTM_TypeOfVal(__VA_ARGS__)); }                                          \
    }
