...
#include "reflection/SourceFile.h"
...
#include "traits/IntTypeBySize.h"
...
#include "reflection/registration/GlobalRegistrer.h"
#include "reflection/registration/Main.h"
#include "traits/FunctionTypeToMethodPointerType.h"
...
RawPlacement<RttiDataMap>             g_pRttiDataMap;
RawPlacement<RttiMutex>               g_RttiMapMutex;
...
void DynamicCppInitializerH::release()
{
    ...
    PHANTOM_WARNING(g_pRttiDataMap->empty(), "Rtti : not every phantom object have been unmapped");
    g_pRttiDataMap.destroy();
...
void elementsByClass(reflection::Class* a_pClass, reflection::LanguageElements& out,
                     phantom::reflection::LanguageElement* a_pScope)
{
    if (a_pClass == nullptr OR Rtti::ClassOf(a_pScope)->isA(a_pClass))
    {
        out.push_back(a_pScope);
    }
    a_pScope->fetchElementsDeep(out, a_pClass);
}
...
PHANTOM_EXPORT_PHANTOM StringHash makeStringHash(StringView a_Str)
{
    StringHash result(crc64(0, (const unsigned char*)a_Str.data(), a_Str.size()));

#define PHANTOM_DEBUG_CHECK_COLLISIONS 0

#if PHANTOM_DEBUG_CHECK_COLLISIONS
