...
struct NewHH<t_Ty, true>
{
    PHANTOM_FORCEINLINE static t_Ty* apply(t_Ty* a_pInstance)
    {
        ...
        a_pInstance->PHANTOM_CUSTOM_EMBEDDED_RTTI_FIELD.metaClass = (reflection::Class*)TypeOf<t_Ty>(TypeOfTag<t_Ty>());
        a_pInstance->PHANTOM_CUSTOM_EMBEDDED_RTTI_FIELD.metaClass->registerInstance(a_pInstance);
        return a_pInstance;
    }
};
...
struct DeleteDynHH<t_Ty, true, true>
{
    static void apply(t_Ty* a_pInstance)
    {
        auto class_ = a_pInstance->PHANTOM_CUSTOM_EMBEDDED_RTTI_FIELD.metaClass;
        auto ptr_ = a_pInstance->PHANTOM_CUSTOM_EMBEDDED_RTTI_FIELD.instance;
        if (auto dfunc = a_pInstance->PHANTOM_CUSTOM_EMBEDDED_RTTI_FIELD.dynamicDeleteFunc)
        {
            dfunc(ptr_);
        }
        else
