...
template<>
struct StringConverter<Path>
{
    static void toLiteral(const reflection::Class*, StringBuffer& a_Buf, const Path* a_pSrc)
    {
        String s = a_pSrc->genericString();
        PHANTOM_TYPE_BY_NAME(String)->valueToLiteral(a_Buf, &s);
    }
    static void to(const reflection::Class*, StringBuffer& a_Buf, const Path* a_pSrc)
    {
        String s = a_pSrc->genericString();
        PHANTOM_TYPE_BY_NAME(String)->valueToString(a_Buf, &s);
    }
    static void from(const reflection::Class*, StringView a_strIn, Path* a_pDest)
    {
        *a_pDest = Path(a_strIn);
    }
};
} // namespace phantom
