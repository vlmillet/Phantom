...
struct Pair
{
    ...
    Pair(Pair<t_OtherFirst, t_OtherSecond> const& a_Other)
        : first(a_Other.first), second(a_Other.second)
    {
    }
    template<class t_OtherFirst, class t_OtherSecond>
    Pair(Pair<t_OtherFirst, t_OtherSecond>&& a_Temp)
        : first(std::move(a_Temp.first)), second(std::move(a_Temp.second))
    {
    }
    t_First  first;
    t_Second second;
...
{
    return Pair<typename std::remove_reference<t_First>::type,
                typename std::remove_reference<t_Second>::type>(std::forward<t_First>(a_First),
                                                                std::forward<t_Second>(a_Second));
}
} // namespace phantom
