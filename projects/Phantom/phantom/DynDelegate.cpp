...
namespace phantom
{
OpaqueDynDelegate::OpaqueDynDelegate(Object a_Object, reflection::Method* a_pMethod)
    : m_pMethod(a_pMethod)
{
    m_pThis = a_Object.as(a_pMethod->getOwnerClass()).getAddress();
...
OpaqueDynDelegate::OpaqueDynDelegate(Object a_Object, StringView a_MethodName)
{
    reflection::Method* pMethod = a_Object.getClass()->getMethodCascade(a_MethodName);
    PHANTOM_ASSERT(pMethod, "no method '%.*s' found in class '%.*s'",
                   PHANTOM_STRING_AS_PRINTF_ARG(a_MethodName),
                   PHANTOM_STRING_AS_PRINTF_ARG(a_Object.getClass()->getName()));
    m_pThis = a_Object.as(pMethod->getOwnerClass()).getAddress();
    m_pMethod = pMethod;
}
...
bool OpaqueDynDelegate::_CheckSignature(reflection::Type*     a_pRetType,
                                        reflection::TypesView a_Types) const
{
    ...
    for (size_t i = 0; i < a_Types.size(); ++i)
    {
        ...
        if (pArgT !=
            ...
            pParamT != PHANTOM_TYPEOF(void*) &&
            pArgT != PHANTOM_TYPEOF(void*)))) // X* -> void* || void* -> X*
            return false;
    }
    return true;
}
