...
#include "reflection/TypeInfos.h"
...
struct DeferredNewH
{
    ...
    inline T* operator*(T* a_pInstance)
    {
        ...
        {
            PHANTOM_VERIFY(a_pInstance->PHANTOM_CUSTOM_EMBEDDED_RTTI_FIELD.metaClass =
                           static_cast<reflection::Class*>(PHANTOM_TYPEOF(T)));
            a_pInstance->PHANTOM_CUSTOM_EMBEDDED_RTTI_FIELD.metaClass->registerInstance(a_pInstance);
        }
        else
        {
            deferInstallation(reflection::TypeInfosOf<T>::object().qualifiedDecoratedName(),
                              &a_pInstance->PHANTOM_CUSTOM_EMBEDDED_RTTI_FIELD);
        }
        return a_pInstance;
    }
};
