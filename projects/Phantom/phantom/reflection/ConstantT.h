...
public:
    void getValue(void* a_pDest) const override
    {
        *reinterpret_cast<ValueTypeNoConst*>(a_pDest) = m_Value;
    }

    bool hasValue(const void* a_pSrc) const override
    {
        return *static_cast<const t_Ty*>(a_pSrc) == m_Value;
    }
    ...
    {
        return PHANTOM_META_NEW(SelfType)(m_strName, m_Value);
    }

    bool isZero() const override
    {
        return m_Value == t_Ty(0);
    }

    bool isIntegral() const override
    {
        return std::is_integral<t_Ty>::value;
    }

    void toString(StringBuffer& a_Buf) const override
    {
        m_pValueType->valueToString(a_Buf, &m_Value);
    }

    void toLiteral(StringBuffer& a_Buf) const override
    {
        m_pValueType->valueToLiteral(a_Buf, &m_Value);
    }
